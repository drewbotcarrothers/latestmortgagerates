...
class LaurentianBankScraper:
    ...
    def scrape(self) -> List[RawRate]:
        ...
        # Fallback rates for Laurentian Bank
        if not rates:
            logger.info(f"Using fallback rates for {self.LENDER_NAME}")
            fallback_rates = [
                (1, Decimal("5.99"), RateType.FIXED),
                (2, Decimal("5.69"), RateType.FIXED),
                (3, Decimal("5.39"), RateType.FIXED),
                (4, Decimal("5.39"), RateType.FIXED),
                (5, Decimal("5.39"), RateType.FIXED),
                (5, Decimal("4.85"), RateType.VARIABLE),
            ]
