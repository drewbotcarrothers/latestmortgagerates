...
class LaurentianBankScraper:
    ...
    def scrape(self) -> List[RawRate]:
        ...
        logger.warning("Laurentian Bank live scraping failed - returning empty list")
        return []
    
    def _scrape_with_playwright(self) -> List[RawRate]:
        """Use Playwright to scrape live rates from Laurentian Bank tables."""
        try:
            ...
            with sync_playwright() as p:
                ...
                # Navigate to Laurentian Bank mortgage rates page
                page.goto(self.RATE_URL, wait_until="domcontentloaded", timeout=30000)
                page.wait_for_timeout(3000)
                
                rates = []
                
                # Laurentian Bank has multiple tables with mortgage rates
                # Fixed-rate, Variable-rate, High-ratio
                tables = page.query_selector_all("table")
                logger.info(f"Found {len(tables)} tables on Laurentian Bank page")
                ...
                for table in tables:
                    ...
                    is_mortgage_table = any(word in header_text for word in ['fixed rate', 'term', 'rate'])
                    
                    if not is_mortgage_table:
                        continue
                    
                    # Determine rate type and mortgage type from headers
                    is_fixed = "fixed" in header_text
                    is_variable = "variable" in header_text
                    is_high_ratio = "high-ratio" in header_text or "high ratio" in header_text
                    ...
                    for row in rows:
                        ...
                        # Parse term - handle various formats
                        # "6 months", "1 year", "2 years", "5-year closed term"
                        term_match = re.search(r'(\d+)\s*(?:months?|month)', term_text, re.IGNORECASE)
                        if term_match:
                        ...
                        if is_high_ratio or 'high-ratio' in term_text.lower() or 'high ratio' in term_text.lower():
                            mortgage_type = MortgageType.INSURED
                            product_name = f"{term_match.group(1)}-Year {rate_type.value.title()} (High Ratio)"
                        else:
