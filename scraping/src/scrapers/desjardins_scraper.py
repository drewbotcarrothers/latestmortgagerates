...
class DesjardinsScraper:
    ...
    def scrape(self) -> List[RawRate]:
        ...
        logger.warning("Desjardins live scraping failed - returning empty list")
        return []
    
    def _scrape_with_playwright(self) -> List[RawRate]:
        """Use Playwright to scrape live rates from Desjardins tables."""
        try:
            ...
            with sync_playwright() as p:
                ...
                # Navigate to Desjardins mortgage rates page
                page.goto(self.RATE_URL, wait_until="domcontentloaded", timeout=30000)
                page.wait_for_timeout(3000)
                
                rates = []
                
                # Desjardins has tables with fixed and variable rates
                tables = page.query_selector_all("table")
                logger.info(f"Found {len(tables)} tables on Desjardins page")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\nScraped {len(rates)} rates from Desjardins:")
        print("-" * 60)
