...
class FirstNationalScraper:
    ...
    def scrape(self) -> List[RawRate]:
        ...
        # Fallback rates for First National (major monoline lender)
        if not rates:
            logger.info(f"Using fallback rates for {self.LENDER_NAME}")
            fallback_rates = [
                (1, Decimal("5.69"), RateType.FIXED),
                (2, Decimal("5.44"), RateType.FIXED),
                (3, Decimal("4.84"), RateType.FIXED),
                (4, Decimal("4.99"), RateType.FIXED),
                (5, Decimal("4.89"), RateType.FIXED),
                (3, Decimal("4.35"), RateType.VARIABLE),
                (5, Decimal("4.15"), RateType.VARIABLE),
            ]
