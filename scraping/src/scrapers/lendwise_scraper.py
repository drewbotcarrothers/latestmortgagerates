...
class LendwiseScraper:
    ...
    def scrape(self) -> List[RawRate]:
        ...
        try:
            ...
            if rates:
                logger.success(f"Successfully scraped {len(rates)} live rates from Lendwise")
                return rates
        except Exception as e:
            logger.warning(f"Playwright scraping failed: {e}")
        
        logger.info("Using fallback rates from Lendwise (Apr 25, 2026)")
                        ...
                        try:
                            ...
                            if 1 <= years <= 10 and 2 <= rate <= 10:
                                rates.append(RawRate(
                                    ...
                                    raw_data={"source": "lendwise_live_scrape", "years": years}
                                ))
                        except:
                            pass
    ...
    def _get_fallback_rates(self) -> List[RawRate]:
        """
        Fallback rates from Lendwise (April 25, 2026).
        Direct mortgage lender popular with online borrowers.
        """
        logger.info("Using fallback rates from Lendwise (Apr 25, 2026)")
        ...
        for item in fallback_data:
            ...
            raw_data = {
                "source": "lendwise_fallback_2026-04-25",
