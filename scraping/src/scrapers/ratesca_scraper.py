...
class RatesCaScraper:
    """Scraper for Rates.ca - Canadian mortgage rate aggregator."""
    ...
    def scrape(self) -> List[RawRate]:
        """Scrape Rates.ca mortgage rates."""
        logger.info("Fetching Rates.ca aggregator page...")
        
        try:
            rates = self._scrape_with_playwright()
            if rates:
                logger.success(f"Successfully scraped {len(rates)} live rates from Rates.ca")
                return rates
                        ...
                        try:
                            ...
                            if 1 <= years <= 10 and 2 <= rate <= 10:
                                rates.append(RawRate(
                                    ...
                                    raw_data={"source": "ratesca_live_scrape", "years": years}
                                ))
                        except:
                            pass
    ...
    try:
        ...
        for lender, lender_rates in sorted(by_lender.items()):
            ...
            for r in sorted(lender_rates, key=lambda x: (x.term_months, x.rate_type.value)):
                ...
                print(f"  {years}yr {r.rate_type.value}: {r.rate}%{featured}")
