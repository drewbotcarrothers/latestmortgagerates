...
class ATBScraper:
    ...
    def scrape(self) -> List[RawRate]:
        ...
        logger.warning("ATB Financial live scraping failed - returning empty list")
        return []
    
    def _scrape_with_playwright(self) -> List[RawRate]:
        """Use Playwright to scrape live rates from ATB Financial tables."""
        try:
            ...
            with sync_playwright() as p:
                ...
                # Navigate to ATB Financial mortgage rates page
                page.goto(self.RATE_URL, wait_until="domcontentloaded", timeout=30000)
                page.wait_for_timeout(3000)
                
                rates = []
                
                # ATB Financial has tables with fixed and variable mortgage rates
                tables = page.query_selector_all("table")
                logger.info(f"Found {len(tables)} tables on ATB Financial page")
