...
class ManulifeBankScraper:
    ...
    def scrape(self) -> List[RawRate]:
        ...
        # Fallback rates for Manulife Bank
        if not rates:
            logger.info(f"Using fallback rates for {self.LENDER_NAME}")
            fallback_rates = [
                (1, Decimal("6.14"), RateType.FIXED),
                (2, Decimal("5.84"), RateType.FIXED),
                (3, Decimal("5.09"), RateType.FIXED),
                (4, Decimal("5.19"), RateType.FIXED),
                (5, Decimal("5.19"), RateType.FIXED),
                (5, Decimal("4.80"), RateType.VARIABLE),
            ]
