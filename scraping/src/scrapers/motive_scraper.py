...
class MotiveScraper:
    ...
    def scrape(self) -> List[RawRate]:
        """Scrape Motive Financial mortgage rates."""
        logger.info("Fetching Motive Financial rate page...")
        
        try:
            rates = self._scrape_with_playwright()
            if rates:
                logger.success(f"Successfully scraped {len(rates)} live rates from Motive Financial")
                return rates
        except Exception as e:
            logger.warning(f"Playwright scraping failed: {e}")
        
        logger.info("Using fallback rates from Motive Financial (2026-07-19)")
                        ...
                        try:
                            ...
                            if 1 <= years <= 10 and 2 <= rate <= 10:
                                rates.append(RawRate(
                                    ...
                                    raw_data={"source": "motive_live_scrape", "years": years}
                                ))
                        except:
                            pass
    ...
    def _get_fallback_rates(self) -> List[RawRate]:
        """
        Fallback rates from Motive Financial (April 25, 2026).
        ...
        fallback_data = [
            ...
            {"term": 60, "type": RateType.FIXED, "rate": "4.49", "mortgage_type": "insured", "product": "5 Year Fixed (Insured)"},
        ...
        for item in fallback_data:
            ...
            raw_data = {
                "source": "motive_fallback_2026-07-19",
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\nScraped {len(rates)} rates from Motive Financial:")
        print("-" * 60)
