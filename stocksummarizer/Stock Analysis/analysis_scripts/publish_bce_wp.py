...
def create_wordpress_post(title, content, featured_image_id=None, status="draft"):
    ...
    try:
        ...
        print(f"SUCCESS - Created post: {result['link']}")
        return result
...
def upload_media_to_wordpress(filepath):
    ...
    try:
        response = urllib.request.urlopen(request)
        result = json.loads(response.read().decode())
        print(f"SUCCESS - Uploaded media: {filepath.name} (ID: {result['id']})")
        return result['id']
    except urllib.error.HTTPError as e:
...
def generate_bce_content(chart_urls):
...
def main():
    ...
    missing = []
    for key, f in chart_files.items():
        if not f.exists():
            missing.append(f.name)
    
    if missing:
        print(f"\nWARNING: Missing chart files: {', '.join(missing)}")
        print("Run create_bce_charts.py first to generate charts.")
        return
    
    print(f"\nFound all chart files")
    ...
    for key, filepath in chart_files.items():
        if filepath.exists():
            ...
            if media_id:
                ...
                chart_urls[key] = f'<p><img src="{WP_URL}/wp-content/uploads/{filepath.name}" alt="BCE Analysis {key}" /></p>'
    ...
    if post:
        ...
        print(f"Edit URL: {WP_URL}/wp-admin/post.php?post={post['id']}&action=edit")
        print("="*80)
        print("\nPost created as DRAFT. Review in WordPress admin before publishing.")
    else:
